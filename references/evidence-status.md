...
- 문단 리듬과 KH-S07의 문장별 분포는 과교정 여부를 다시 읽기 위한 관찰값이다. 한 문장 문단 비율·연결어미 총계·강조 비율에 보편 상한을 붙이지 않는다.
- `rule_guided_draft`와 장르 allowlist는 실사용에서 드러난 작업 범위의 누락·반복 재판정 비용을 줄이는 실행 계약이다. 이들은 AI 표지의 정밀도나 장르별 정상 분포를 증명하지 않는다.
