...
- 42개 분류 항목 중 27개만 결정적 위치 앵커를 가진다. 나머지는 문단·리듬·서식·용어 이관·저자 피드백 맥락을 사람이 판정하는 항목이다.
...
- KH-S05 구체화와 KH-S42 자기해설은 서로 반대 방향의 문맥 규칙이다. 공개 스킬에는 비공개 문장·채택률을 싣지 않고, `같은 문단에서 답을 찾을 수 있는가`와 `지웠을 때 새 정보가 사라지는가`라는 판정 경계만 둔다.
- 저자 피드백 교정은 프로젝트 내부 대장으로만 사용한다. 반복 채택·기각은 다음 제안의 로컬 경계를 좁히지만 보편 taxonomy의 정밀도 근거가 아니다.
- 강조 제거 후 감사는 제거 전후가 같은 문단의 짧은 선두 볼드만 위치 후보로 모집한다. 목록 전환·문장화·보존은 자동 판정하지 않는다.
...
비공개 원고의 반복 검토는 KH-S01·S07·S34·S41의 처방을 좁히는 데 사용했다. 그러나 저자·원고·문장·횟수·기각률·절대 경로는 공개 저장소에 넣지 않는다. 이 관찰에서 공개 규칙으로 옮길 수 있는 것은 다음 경계뿐이다.

- KH-S01의 대조, KH-S07의 쉼표, KH-S34의 청유·평서 도입, KH-S41의 명사형 되받기는 모두 자동 수정 대상이 아니라 문맥 후보로 남긴다.
- 독자의 길잡이가 되는 도입문, 실제 호흡을 만드는 쉼표, 정착한 명사, 원고가 정한 용어는 보존 반례다.
- 스캔 경계가 바뀌면 계측값도 바뀐다. 주석·인용·표·목록·보호 구간의 제외 기준과 입력 내력을 receipt에 남기며, 수치를 보편 임계값으로 쓰지 않는다.
- 규칙 문서를 읽고 쓴 초고는 `human_draft`가 아니라 `rule_guided_draft`로 기록한다. 적용 범위 밖 규칙은 `scope_warnings`로 확인한다.

## 독자·장문 전개 연구의 반영

- 독자군에 따라 좋은 글의 기준이 달라질 수 있다는 연구는 사용자 지정 `reader_profile`과 맹검 사람 평가의 필요성을 뒷받침한다. [Marco 외 2025](https://aclanthology.org/2025.findings-acl.1304/)
- 장문 창작물은 문장 단위의 자연스러움만으로 평가하기 어렵다는 연구는 선택적 `trajectory_ledger`와 장문 재독 계약의 근거다. [Wang 외 2025](https://aclanthology.org/2025.findings-acl.1114/), [Yang·Jin 2025](https://aclanthology.org/2025.acl-long.799/)
- 모델 심사와 사람 선호의 불일치는 모델을 최종 심사자가 아니라 후보·누락 탐지 보조로 제한해야 한다는 근거다. [Fein 외 2026](https://aclanthology.org/2026.eacl-long.362/)
