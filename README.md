...
> 상태: v0.11 prototype. 42개 문체·구조 후보, 27개 결정적 위치 앵커, 무수정·신규 표지 gate, 빈 추상명사 구체화·자기해설 문단 판정, 문단 리듬·문장 내 연결 연쇄 관찰, 장르 allowlist, 프로젝트 문체 프로필·저자 피드백 교정 대장, Markdown·DOCX 형식 프로필과 강조 제거 후 잔재 감사, 반복 윤문 수렴 계약, 강점 보존·축약 불가 연쇄, 의미 중복 삼항 연쇄, 다파일 구조 분포·다의어 선별 이관 감사, 선택적 독자 적합성·장문 전개 대장과 전방 비교 검증을 갖췄습니다. 독립적인 맹검 사람 평가는 계속 진행 중입니다.
...
| 삼항 반복 검토 | `-고/-며`로 이어진 세 항의 위치를 찾고, 같은 평가의 유사어 반복인지 실제 단계·조건인지 사람이 판정합니다. 숫자 셋만으로 삭제하지 않습니다. |
...
| 삭제와 구체화의 균형 | 새 정보 없는 자기해설은 문단 단위로 덜고, 논증을 떠맡은 빈 추상명사는 같은 문단의 내용으로 채웁니다. |
| 반복 저자 검토 | 채택·기각 이유를 프로젝트 내부 대장에 남겨 다음 제안의 경계를 좁히고, 낮은 확신의 후보로 목록을 채우지 않습니다. |
...
| 장문 원고 | manifest와 review ledger로 실제 검사·판정 범위를 남기고, 요청이 있을 때만 독자 적합성·장면·논지의 전개를 선택적 대장으로 검토합니다. |
| 안전·비교 검사 | 인용·수치·날짜·조문을 보호하고, 윤문 후보는 같은 기준으로 비교합니다. |
| 반복 검수 수렴 | 최초본과 직전본을 함께 대조하고, 새 결함이 없으면 억지로 다시 쓰지 않습니다. |
| 용어 이관 감사 | 확정 용어를 잠근 뒤 기존어 잔존과 치환 후 동어반복·호응·연어 후보를 다시 검토합니다. |
...
- 의미 명세에서 직접 확인·출처의 주장·전언·추정·유보·반대 심문의 지위를 구분합니다.
...
사용자가 볼드 제거를 명시했다면 제거 뒤 짧은 라벨이 맨몸으로 남거나 연속 나열의 구조가 흐려지지 않았는지 별도로 감사합니다.

    python3 scripts/audit_emphasis_removal.py --before draft.md --after edited.md
...
장문에서 누구에게 어떻게 읽히는지 또는 장·절의 전개를 함께 검토하려면 독자군·읽는 상황·우선순위 2~4개를 함께 알려 주세요. 선택적으로 [독자 적합성·장문 전개 검토](references/reader-fit-and-trajectory.md)의 대장을 써서, 열린 결말을 억지 결론으로 바꾸지 않았는지와 앞선 약속·전환이 사라지지 않았는지를 사람이 대조합니다.
